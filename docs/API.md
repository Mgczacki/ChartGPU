...
- **`CandlestickSeriesConfig`**: extends shared series fields with `type: 'candlestick'` and OHLC-specific configuration. See [`types.ts`](../src/config/types.ts).
  ...
  - **Current limitations (important)**: candlestick rendering is not yet implemented (the render coordinator skips candlestick series with a warning); streaming append via `appendData(...)` is not supported for candlestick series; candlestick series are excluded from cartesian hit-testing utilities (tooltip/hover/crosshair interactions will not detect candlestick data points). See [`createRenderCoordinator.ts`](../src/core/createRenderCoordinator.ts).
